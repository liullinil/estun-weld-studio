...
The export dialog defaults to retaining arcs and torch digital output 1. It can linearize arcs and choose output 0–65535; it reruns only postprocessing, without complete replanning. The displayed and simulated path updates to the validated export variant.

Weaving is off by default, with detailed defaults: one-sided amplitude 1 mm, frequency 2 Hz, plane rotation 0°, endpoint fade 2 mm. Enabling it produces an actual sinusoidal IK/collision-checked path for playback and export. Unsafe or unreachable weave requests fail explicitly. Export uses explicit validated samples because the supplied `movLW/movCW` reference does not specify waveform/type/frame semantics sufficiently to reproduce them exactly.

The circular collision icon on the left enables/disables live red highlighting of implicated robot links and torch. Native checks include the workpiece, floor, plinth and nonadjacent self-collisions; same/adjacent links are excluded. It polls only changed poses, at most five requests per second, and reuses a bounded server collision scene. Turning the icon off restores original materials and stops visualization queries; planner collision checks remain enabled.
...
- Current Docker image `estun-hyper:20260916-selection-collision`; container `hyper-app-v5`.
...
- Current complete release: `/srv/hyper/releases/20260916-selection-collision`. Caddy `handle_path /hyper/*` forwards to `hyper-app-v5:18740`; the existing fallback still forwards to `web:80`.
