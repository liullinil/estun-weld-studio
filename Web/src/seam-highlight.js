...
export function selectedSeamOverlay(points,id,color=SELECTED_SEAM_COLOR){
  const layers=[];
  for(const [width,opacity,order] of [[11,.22,30],[5,1,31]]){
    const geometry=new LineGeometry().setPositions(points);
    const material=new LineMaterial({color,linewidth:width,worldUnits:false,transparent:true,opacity,depthTest:false,depthWrite:false,toneMapped:false});
