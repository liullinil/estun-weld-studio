export function angleFromPoint(x,y){return Math.max(0,Math.min(180,Math.atan2(Math.max(0,108-y),140-x)*180/Math.PI));}
export function createAngleRange(container,minInput,maxInput,onChange){
  ...
  el('path',{d:'M207 108 H140 V41',class:'angle-profile'});el('path',{d:'M154 108 V94 H140',class:'right-angle-mark'});
  ...
  for(const [index,label] of [[0,'Minimum angle'],[1,'Maximum angle']]){
    ...
    // Only a deliberate primary press on a handle starts a drag. Keep listeners
    // on window and capture on a stable element, never moved during update().
    knob.addEventListener('pointerdown',e=>{if(e.button!==0||container.hasAttribute('inert'))return;e.preventDefault();e.stopPropagation();pointer=e.pointerId;knob.setPointerCapture(pointer);change(e);});
    window.addEventListener('pointermove',e=>{if(pointer!==e.pointerId)return;if((e.buttons&1)===0){pointer=null;return;}e.preventDefault();change(e);},{passive:false});
    const release=e=>{if(pointer!==e.pointerId)return;pointer=null;if(knob.hasPointerCapture(e.pointerId))knob.releasePointerCapture(e.pointerId);};
    window.addEventListener('pointerup',release);window.addEventListener('pointercancel',release);knob.addEventListener('lostpointercapture',()=>{pointer=null;});window.addEventListener('blur',()=>{pointer=null;});
  ...
  for(const h of handles)svg.append(h.knob);container.replaceChildren(svg);update();return {update};
}
