...
import { HOME, TOOL, TOOL_DISPLAY, jointMatrix } from './kinematics.js';
...
import { isRedSeam, warningReason } from './warnings.js';
...
export function createStudio(container,onTransform,onSeam){
  ...
  const camera=new T.PerspectiveCamera(38,1,.001,50),orbit=new OrbitControls(camera,renderer.domElement);orbit.enableDamping=true;orbit.dampingFactor=.12;orbit.minDistance=0;orbit.maxDistance=Infinity;orbit.zoomToCursor=true;orbit.maxPolarAngle=Math.PI;orbit.mouseButtons={LEFT:T.MOUSE.ROTATE,MIDDLE:T.MOUSE.PAN,RIGHT:T.MOUSE.ROTATE};
  ...
  const axes=new T.AxesHelper(.16);axes.applyMatrix4(TOOL_DISPLAY);axes.material.depthTest=false;axes.renderOrder=20;tcp.add(axes);
  ...
  const part=new T.Group();scene.add(part);let seamGroup=new T.Group();part.add(seamGroup);let partMesh,selection=null,simulating=false;
  ...
  const seamHint=document.createElement('div');seamHint.className='viewport-seam-hint hidden';container.append(seamHint);
  function pickSeam(e){const r=renderer.domElement.getBoundingClientRect();raycaster.params.Line.threshold=camera.position.distanceTo(orbit.target)*2*Math.tan(T.MathUtils.degToRad(camera.fov/2))*5/Math.max(1,r.height);raycaster.setFromCamera(new T.Vector2((e.clientX-r.left)/r.width*2-1,-(e.clientY-r.top)/r.height*2+1),camera);return raycaster.intersectObjects(seamGroup.children).sort((a,b)=>raycaster.ray.distanceSqToPoint(a.point)-raycaster.ray.distanceSqToPoint(b.point)||a.distance-b.distance)[0];}
  renderer.domElement.addEventListener('pointerdown',e=>{if(e.button===0)down=[e.clientX,e.clientY];seamHint.classList.add('hidden');});renderer.domElement.addEventListener('pointerup',e=>{const pressed=down;down=null;if(e.button!==0||simulating||!pressed||Math.hypot(e.clientX-pressed[0],e.clientY-pressed[1])>4||transform.dragging)return;const hit=pickSeam(e);if(hit)onSeam(hit.object.userData.id,{x:e.clientX,y:e.clientY});});
  renderer.domElement.addEventListener('pointercancel',()=>{down=null;});
  renderer.domElement.addEventListener('pointermove',e=>{if(simulating||e.buttons||transform.dragging){seamHint.classList.add('hidden');return;}const hit=pickSeam(e),reason=hit?.object.userData.warning;if(!reason){seamHint.classList.add('hidden');return;}const r=container.getBoundingClientRect();seamHint.textContent=reason;seamHint.style.left=`${Math.max(8,Math.min(r.width-280,e.clientX-r.left+14))}px`;seamHint.style.top=`${Math.max(8,Math.min(r.height-90,e.clientY-r.top+14))}px`;seamHint.classList.remove('hidden');});
  renderer.domElement.addEventListener('pointerleave',()=>seamHint.classList.add('hidden'));
  renderer.domElement.addEventListener('wheel',e=>{if(!partMesh)return;pickSeam(e);const hit=raycaster.intersectObject(partMesh)[0];if(hit){const forward=camera.getWorldDirection(new T.Vector3()),depth=hit.point.clone().sub(camera.position).dot(forward);if(depth>0)orbit.target.copy(camera.position).addScaledVector(forward,depth);}},{capture:true,passive:true});
  ...
  function loadPart(doc){transform.detach();if(partMesh){part.remove(partMesh);partMesh.geometry.dispose();partMesh.material.dispose();}const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(doc.vertices,3));geo.setAttribute('normal',new T.Float32BufferAttribute(doc.normals,3));const indices=doc.indices.slice();for(let i=0;i<indices.length;i+=3)[indices[i+1],indices[i+2]]=[indices[i+2],indices[i+1]];geo.setIndex(indices);geo.computeBoundingBox();const partMaterial=mat('#98abb9',.65,.32);partMaterial.side=T.DoubleSide;partMesh=mesh(geo,partMaterial,part);const center=geo.boundingBox.getCenter(new T.Vector3());part.position.set(.85-center.x,.27-geo.boundingBox.min.y,-center.z);part.rotation.set(0,0,0,'YXZ');transform.attach(part);clearEffects();return part;}
  function seams(list,results,selected,included=new Set()){seamHint.classList.add('hidden');selection=selected;for(const c of [...seamGroup.children]){c.geometry.dispose();c.material.dispose();seamGroup.remove(c);}for(const s of list){const chosen=included.has(s.id),result=results?.find(r=>r.id===s.id),warning=isRedSeam(result,chosen),color=warning?'#ff3657':chosen?'#00ffff':'#e8b661';const geo=new T.BufferGeometry().setAttribute('position',new T.Float32BufferAttribute(s.points,3)),line=new T.Line(geo,new T.LineBasicMaterial({color,depthTest:false,transparent:true,opacity:chosen?1:.68,toneMapped:false}));line.renderOrder=8;line.userData.id=s.id;line.userData.warning=warning?warningReason(result):'';seamGroup.add(line);if(chosen||warning)for(const overlay of selectedSeamOverlay(s.points,s.id,warning?'#ff3657':'#00ffff'))seamGroup.add(overlay);}}
  ...
  return {renderer,scene,camera,orbit,navigation,part,transform,tcp,loadRobot,loadPart,seams,render,fit,clearEffects,showProgram,showCollisions,
    ...
    setSimulation(active){simulating=active;if(active)seamHint.classList.add('hidden');seamGroup.visible=!active;plannedPath.visible=!active;},
