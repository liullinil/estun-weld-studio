...
export function createStudio(container,onTransform,onSeam){
  ...
  function seams(list,results,selected){selection=selected;for(const c of [...seamGroup.children]){c.geometry.dispose();c.material.dispose();seamGroup.remove(c);}for(const s of list){const result=results?.find(r=>r.id===s.id),color=s.id===selection?'#ffde8b':result?(result.state==='Ready'?'#62dda0':'#ee7478'):'#e8b661';const geo=new T.BufferGeometry().setAttribute('position',new T.Float32BufferAttribute(s.points,3)),line=new T.Line(geo,new T.LineBasicMaterial({color,depthTest:false,transparent:true,opacity:.92}));line.renderOrder=8;line.userData.id=s.id;seamGroup.add(line);}}
