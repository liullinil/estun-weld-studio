...
export function createStudio(container,onTransform,onSeam){
  ...
  for(const s of [-1,1]){segment([s*.024,-.018,.019],[s*.024,.016,.026],.005,graphite);segment([s*.024,-.023,.019],[s*.024,-.031,.019],.006,s===1?blue:copper);}
  ...
  function seams(list,results,selected){selection=selected;for(const c of [...seamGroup.children]){c.geometry.dispose();c.material.dispose();seamGroup.remove(c);}for(const s of list){const result=results?.find(r=>r.id===s.id),color=s.id===selection?'#00ffff':result?(result.state==='Ready'?'#62dda0':'#ee7478'):'#e8b661';const geo=new T.BufferGeometry().setAttribute('position',new T.Float32BufferAttribute(s.points,3)),line=new T.Line(geo,new T.LineBasicMaterial({color,depthTest:false,transparent:true,opacity:s.id===selection?1:.72,toneMapped:false}));line.renderOrder=8;line.userData.id=s.id;seamGroup.add(line);if(s.id===selection)for(const overlay of selectedSeamOverlay(s.points,s.id))seamGroup.add(overlay);}}
