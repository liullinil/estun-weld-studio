...
import { isWarning } from './warnings.js';
...
export function createStudio(container,onTransform,onSeam){
  ...
  function seams(list,results,selected,included=new Set()){selection=selected;for(const c of [...seamGroup.children]){c.geometry.dispose();c.material.dispose();seamGroup.remove(c);}for(const s of list){const chosen=included.has(s.id),result=results?.find(r=>r.id===s.id),warning=isWarning(result),color=warning?'#ff3657':chosen?'#00ffff':result?(result.state==='Ready'?'#62dda0':'#ee7478'):'#e8b661';const geo=new T.BufferGeometry().setAttribute('position',new T.Float32BufferAttribute(s.points,3)),line=new T.Line(geo,new T.LineBasicMaterial({color,depthTest:false,transparent:true,opacity:chosen?1:.68,toneMapped:false}));line.renderOrder=8;line.userData.id=s.id;seamGroup.add(line);if(chosen||warning)for(const overlay of selectedSeamOverlay(s.points,s.id,warning?'#ff3657':'#00ffff'))seamGroup.add(overlay);}}
  ...
  return {renderer,scene,camera,orbit,navigation,part,transform,tcp,loadRobot,loadPart,seams,render,fit,clearEffects,showProgram,showCollisions,
    ...
    setSimulation(active){simulating=active;seamGroup.visible=!active;plannedPath.visible=!active;},
