export class ProgramPlayback {
  constructor(){this.program=null;this.state='manual';this.index=0;this.elapsed=0;this.start=[];this.angles=[];this.arc=false;}
  get automatic(){return this.state==='playing'||this.state==='paused';}
  get ready(){return !!this.program?.moves?.length&&this.state==='ready';}
  load(program){this.program=program;this.state=program?.moves?.length?'ready':'manual';this.arc=false;}
  ...
  stop(){this.program=null;this.state='manual';this.arc=false;}
  tick(dt,override){
