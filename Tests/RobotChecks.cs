...
public partial class RobotChecks : Node
{
    ...
    private void CheckControllerInterlocks()
    {
        ...
        Tick(controller, 30);
        Require(controller.AnglesDegrees.SequenceEqual(before) && !controller.MotionActive && controller.CanMove, "Jog release stops motion while drives remain ready");
        controller.SetJointJog(0, -1);
        controller.EmergencyStop();
        before = controller.AnglesDegrees;
        Tick(controller, 30);
        Require(controller.EmergencyStopped && !controller.DrivesEnabled && !controller.CanMove, "Emergency stop disables drives and blocks all motion");
        ...
        Require(controller.GoHome(), "Home starts with drives on and no Space hold");
        ...
        Require(controller.PlayWaypoints(), "Program starts with drives on and no Space hold");
        ...
        before = controller.AnglesDegrees;
        Tick(controller, 30);
        Require(controller.DrivesEnabled && !controller.MotionActive && controller.AnglesDegrees.SequenceEqual(before), "Application focus loss cancels motion even without an enable-switch transition");
        controller._Notification((int)NotificationApplicationFocusIn);
        Tick(controller, 30);
        Require(!controller.MotionActive && controller.AnglesDegrees.SequenceEqual(before), "Focus return does not resume cancelled motion");
        Require(controller.ApplyPlannedPose(RobotController.HomeAngles, "Planned test motion"), "Validated planned poses run with drives on and no Space hold");
        controller._Notification((int)NotificationApplicationFocusOut);
        Require(controller.DrivesEnabled && !controller.MotionActive && !controller.IsPlannedMotion, "Focus loss clears the external planned-motion session");
        ...
        Require(!controller.GoHome() && !controller.PlayWaypoints() && !controller.ApplyPlannedPose(RobotController.HomeAngles, "Blocked motion"), "Home, program and planned poses still obey the drives interlock");
        controller.Free();
        root.Free();
    }
